...
function Orders() {
      ...
      try {
        ...
        const normalized = ordersArray.map((order) => {
          ...
          const itemsNorm = items.map((it) => ({
            ...
            length: it.length ?? "",
            weight: it.weight ?? "",
            mahaveer_price: it.mahaveer_price ?? "",
        ...
        setCompleted((prev) => {
          const next = { ...prev };
          for (const o of normalized) {
            if ((o.fulfill_status || "").toLowerCase() === "completed") next[o.id] = true;
          }
          return next;
        });
      } catch (err) {
  ...
  const getCustomerType = (order) => {
    ...
    if (
      domain &&
      !["gmail.com", "yahoo.com", "outlook.com", "hotmail.com", "icloud.com", "proton.me"].includes(domain.toLowerCase())
    )
      return "B2B";
    return "B2C";
  };
        ...
        list.map((o) =>
          o.id === id
            ? {
                ...
                decision_status: data.order?.decision_status || o.decision_status || "Accepted",
              }
            : o
        )
      );
        ...
        <div className="orders-toolbar">
          ...
          <div className="status-filters">
            {Object.keys(statusFilters).map((key) => (
              <label key={key} className={`chip ${statusFilters[key] ? "on" : "off"}`}>
                <input type="checkbox" checked={statusFilters[key]} onChange={() => toggleFilter(key)} />
                {key}
              </label>
            ))}
          </div>
                ...
                filteredAndSortedOrders.flatMap((order) => {
                    ...
                    return (
                      <tr key={`${order.id}-empty`} className="order-row" onClick={() => handleRowClick(order)}>
                        ...
                        <td>
                          <span
                            className={`badge ${
                              getEffectiveStatus(order).toLowerCase() === "completed"
                                ? "order-completed"
                                : getEffectiveStatus(order).toLowerCase()
                            }`}
                          >
                  ...
                  return items.map((it, idx) => (
                    <tr key={`${order.id}-${idx}`} className="order-row" onClick={() => handleRowClick(order)}>
                      ...
                      <td>
                        {it.image_url && <img src={it.image_url} alt={it.product_name || "item"} className="thumbnail" />}
                      </td>
                      <td>{it.quantity ?? ""}</td>
                    ...
                    <div className="item-card" key={i}>
                      ...
                      <div className="item-right">
                        ...
                        <div className="item-meta">
                          <span>Qty: {fmtNum(it.quantity)}</span>
                          <span>{priceItem(it.unit_price_minor)}</span>
                        </div>
                        <div className="spec-grid">
                          <div className="spec-cell">
                            <span>HSN%</span>
                            <strong>{fmtPct(it.hsn_percentage)}</strong>
                          </div>
                          <div className="spec-cell">
                            <span>Mahaveer</span>
                            <strong>{fmtMoney(it.mahaveer_price)}</strong>
                          </div>
                          <div className="spec-cell">
                            <span>MRP</span>
                            <strong>{fmtMoney(it.mrp)}</strong>
                          </div>
                          <div className="spec-cell">
                            <span>Length</span>
                            <strong>{fmtNum(it.length)}</strong>
                          </div>
                          <div className="spec-cell">
                            <span>Width</span>
                            <strong>{fmtNum(it.width)}</strong>
                          </div>
                          <div className="spec-cell">
                            <span>Height</span>
                            <strong>{fmtNum(it.height)}</strong>
                          </div>
                          <div className="spec-cell spec-span-2">
                            <span>Weight</span>
                            <strong>{fmtNum(it.weight)}</strong>
                          </div>
                        </div>
                      </div>
                    </div>
            ...
            <div className="modal-footer">
              <div className="left-note">
                ...
                (selectedOrder.fulfill_status || "").toLowerCase() === "completed" ? (
                  ...
                  selectedOrder.decision_status === "Declined" ? (
              ...
              <div className="actions">
                {decisions[selectedOrder.id] === "Accepted" ||
                selectedOrder.decision_status === "Accepted" ? null : (
                  <>
                    <button
                    ...
                    >
                      {saving[selectedOrder.id] && decisions[selectedOrder.id] !== "Accepted"
                        ? "Saving..."
                        : "Decline"}
                    </button>
                    <button
                    ...
                    >
                      {saving[selectedOrder.id] && decisions[selectedOrder.id] !== "Declined"
                        ? "Saving..."
                        : "Accept"}
                    </button>
                  </>
